<?PHP
	...
	while ($keepgoing == true) {
		...
		if (!isset($tmp[499])) $keepgoing = false;
	}
	echo 'Total revs: '.count($revs).".\n";

	foreach ($revs as $data) {
		$es = strtolower($data['comment']);
		if (fnmatch('reverting *',$es)) $count++;
		if (fnmatch('reverting',$es)) $count++;
		if (fnmatch('reverted *',$es)) $count++;
		if (fnmatch('reverted',$es)) $count++;
		if (fnmatch('revert *',$es)) $count++;
		if (fnmatch('revert',$es)) $count++;
		if (fnmatch('rvt *',$es)) $count++;
		if (fnmatch('rvt',$es)) $count++;
		if (fnmatch('rv *',$es)) $count++;
		if (fnmatch('rv',$es)) $count++;
		if (fnmatch('rvv *',$es)) $count++;
		if (fnmatch('rvv',$es)) $count++;
		if (fnmatch('*undid*',$es)) $count++;
	}
