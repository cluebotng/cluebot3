...
class WikipediaIndex
{
    ...
    public function forcepost(
        ...
        if ($maxlag != null) {
            $x = $this->http->post($this->indexurl . '?title=' . urlencode($page) .
                                   '&action=submit&maxlag=' . urlencode($maxlag) . '&bot=' .
                                   (($bot == true) ? '1' : '0'), $post);
            if (preg_match('/Waiting for ([^ ]*): ([0-9.-]+) seconds lagged/S', $x, $lagged)) {
                ...
                if ($mlkg != true) {
                    ...
                    $x = $this->http->post($this->indexurl . '?title=' . urlencode($page) .
                                           '&action=submit&bot=' . (($bot == true) ? '1' : '0'), $post);
                }
            }

            return $x;
        } else {
            return $this->http->post($this->indexurl . '?title=' . urlencode($page) .
                                     '&action=submit&bot=' . (($bot == true) ? '1' : '0'), $post);
        }
    }
    ...
    public function diff($title, $oldid, $id, $wait = true)
    {
        ...
        $url = $this->indexurl . '?title=' . urlencode($title) . '&action=render&diff=' .
    ...
    public function rollback($title, $user, $reason = null, $token = null, $bot = true)
    {
        ...
        $x = $this->http->get($this->indexurl . '?title=' . urlencode($title) .
                              '&action=rollback&from=' . urlencode($user) . '&token=' .
                              urlencode($token) . (($reason != null) ? '&summary=' . urlencode($reason) : '') .
                              '&bot=' . (($bot == true) ? '1' : '0'));
    ...
    public function email($user, $subject, $body)
    {
        ...
        return $this->http->post($this->indexurl . '?title=Special:EmailUser&target=' .
                                 urlencode($user) . '&action=submit', $post);
    }
}
