...
function process_page($page)
{
    ...
    if ($pagedata = $wpq->getpage($page)) {
        foreach (Config\find_config_blocks($user, $pagedata) as $config_block) {
            $config = Config\build_config_from_config_block($page, $config_block);
            if (!$config->isValid()) {
                $logger->warning("Skipping invalid config on " . $page . ": " . $config_block->text);
                continue;
            }
